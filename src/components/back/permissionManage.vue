<template>
  <div style="margin-top:10px;">
    ...
    <el-table size="small" :data="listData" highlight-current-row v-loading="loading" border element-loading-text="拼命加载中" style="width: 100%;">
      ...
      <el-table-column sortable prop="name" label="权限名" width="150">
      </el-table-column>
      <el-table-column sortable prop="url" label="接口地址" width="200">
      </el-table-column>
      <el-table-column sortable prop="permission" label="权限" width="150">
      </el-table-column>
      <el-table-column align="center" label="操作" min-width="300">
        <template slot-scope="scope">
          <el-button size="mini" @click="handleEdit(scope.$index, scope.row)">编辑</el-button>
          <el-button size="mini" type="danger" @click="deletePermission(scope.$index, scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
    ...
    <el-dialog :title="title" :visible.sync="editFormVisible" width="30%" @click='closeDialog("edit")'>
      <el-form label-width="120px" :model="editForm" ref="editForm" :rules="rules">
        ...
        <el-form-item label="权限" prop="permission">
          <el-input size="small" v-model="editForm.permission" auto-complete="off" ></el-input>
        </el-form-item>
      </el-form>
...
export default {
  data() {
    return {
      ...
      rules: {
        url: [
          { required: true, message: '输入接口地址', trigger: 'blur' }
        ],
        name: [
  ...
  methods: {
    // 获取角色列表
    getdata() {
      this.$http.get('/api/back/getPermissionListByPage',{params:{
    ...
    submitForm(editData) {
      this.$refs[editData].validate(valid => {
        if (valid) {
          this.$http.post('/api/back/addOrUpdatePermission',this.editForm).then(resp=>{
            if(resp.data.code=='000000'){
    ...
    deletePermission(index, row) {
      this.$confirm('确定要删除吗?', '信息', {
        ...
        .then(() => {
          this.$http.delete('/api/back/deletePermission',{
            params:{
              id:row.id
            }
          })
